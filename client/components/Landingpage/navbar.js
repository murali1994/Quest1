    ...
    checkForFailedSearchAlert() {
      ...
      search(){
        ...
        if(that.state.searchValue == '' ){
          ...
          alert("Enter the question"+that.state.searchValue);
        }
        else{
        $.ajax({
          url:'/search',
          ...
          success: function(answers) {
            ...
            var answers;
            console.log(answers);
            if(answers.status == 'No answers'){
              if(that.state.token){
                console.log("----------"+token);
                var qid=answers.questionId;
                console.log(qid);
                that.setState({openAnswer:true})
                noAnswers = <Postanswer toaster={that.refs.toasterContainer} qid={qid}/>
              }
              else{
                that.checkForNoAnswerForQuestionValues();
              }
              that.setState({noAnswers:noAnswers})
            }
            else{
               answers = answers.map((row,index)=> {
               that.setState({question:row.question});
               return <IndividualQuestion answer={row.answer} answered_by={row.answered_by} likes={row.likes} dislikes={row.dislikes} timestamp={row.time}  picture={row.picture} key = {index}/>
             });

             that.setState({answers : answers,redirect:true});
           }
          },
          error: function(err) {
                   ...
                   <div className="input-group-btn">
                       ...
                       <Dialog
                         ...
                         {this.state.redirect?<Redirect to={{
                             pathname: '/displayAnswer/'+this.state.question,
                 ...
                 <button type="button" className=" btn btn-round-lg btn-sm  btnColor" onClick={this.askQuestion.bind(this)}>Ask Question <i className="fa fa-question" aria-hidden="true"></i>
                   <Dialog
                     ...
                     >
                       ...
                       <AskQuestion handleClose={this.handleClose.bind(this)} toaster={this.refs.asd}/>
                     </Dialog>
                 </button>
       ...
       <div className="modal-dialog" role="document">
          <div className="modal-content">
             ...
             <div className="modal-body">
                <h2 style={{marginLeft:'100px',marginBottom:'50px',fontFamily:'Roboto'}}> Login to Quest</h2>
                <form>
                   <div >
